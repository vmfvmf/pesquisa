...
echo $this->Html->script('jquery-ui', false); ?>
    ...
    $scrip .= '];
            var manifest = [';
    foreach($imgs as $img){
        for($i=1;$i<=$img["Tipoobjeto"]["max_rotacao"];$i++)$scrip .= '{id:"'.$img["Tipoobjeto"]["img_base"].$i.'", '
                . 'src:"http://localhost/biblioteca/img/gifs/'.$img["Tipoobjeto"]["img_base"].$i.'.gif", cod:"'.$img["Tipoobjeto"]["id"].'" },';
        ...
        if(isset($idObj)){
            $scrip .= '
                if(bitmap.name =='.$idObj.'){
                    ...
                    $.post("http://localhost/biblioteca/Bibliotecas/loadobj",{id:'.$idObj.'},
                                function( data ) {
